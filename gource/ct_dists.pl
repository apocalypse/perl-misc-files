...
# Warning: the resulting log will be ~700M as of March 20, 2010!
# -rw-r--r-- 1 apoc apoc 677M 2010-03-20 22:45 ct_dists.log
...
sub main {
	...
	my $sth = $dbh->prepare( 'SELECT date, dist, tester FROM cpanstats ORDER BY date' );
	$sth->execute;
	my $newdata;
...
perl ct_dists.pl > ct_dists.log

# Then run Gource like this:
gource ct_dists.log -1280x720 --highlight-all-users --multi-sampling \
